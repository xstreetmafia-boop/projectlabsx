// Single source of truth for site-wide SEO values. Update SITE_URL here if
// the domain ever changes, and every page's metadata, sitemap, robots, and
// JSON-LD will follow automatically.
export const SITE_URL = "https://projectlabsx.vercel.app";
export const SITE_NAME = "Project Labs X";
export const SITE_TITLE = "Project Labs X — Design, Development & AI Research Studio";
export const SITE_DESCRIPTION =
  "Project Labs X is a design, development, and AI research studio based in Kozhikode, Kerala. Brand identity, web design, product design, and PLX-X1 AI research.";
export const SITE_EMAIL = "projecthekalabs@gmail.com";
export const SITE_PHONE = "+917356640404";
export const SITE_ADDRESS = {
  streetAddress: "5V9G+J7G Nisari Mall, Nisari Junction",
  addressLocality: "Ramanattukara, Kozhikode",
  addressRegion: "Kerala",
  postalCode: "673634",
  addressCountry: "IN",
};
